...
export default function PariwisataPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [pariwisataRes, dokumentasiRes] = await Promise.all([
          fetch('/api/pariwisata', { next: { revalidate: 10 } }),
          fetch('/api/dokumentasi', { next: { revalidate: 10 } })
        ])
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      {data.length === 0 ? (
        <div className="text-center text-gray-500 py-12 bg-white rounded-lg shadow p-6">
              ...
              <div key={doc.id} className="group relative overflow-hidden rounded-lg">
                ...
                <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <div className="text-white text-center p-4">
                    <h3 className="font-semibold">{doc.nama}</h3>
                    <p className="text-sm line-clamp-2">{doc.deskripsi}</p>
                  </div>
                </div>
              </div>
            ))}
